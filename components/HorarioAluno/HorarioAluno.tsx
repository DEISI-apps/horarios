...
export default function HorarioAluno() {
  ...
  function descarregaTurmas() {
    ...
    setDownloadFn(null);
    setAlunoInfo(null);
    fetch(`https://horariosdeisi.pythonanywhere.com/aluno-turmas/${numeroAluno}`)
      .then((res) => res.json())
      .then((data) => {
        setAlunoInfo(data);
        setIsLoading(false);
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 rounded-2xl shadow-lg text-white">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-2xl font-bold shadow-inner border-2 border-white/30">
                {alunoInfo.aluno.charAt(0).toUpperCase()}
              </div>
              <div className="flex flex-col">
                <h2 className="text-2xl font-bold">{alunoInfo.aluno}</h2>
            ...
            {downloadFn && (
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <button
                  onClick={downloadFn}
                  className="flex-1 sm:flex-none px-6 py-3 bg-white text-blue-600 hover:bg-blue-50 rounded-xl flex items-center justify-center gap-2 font-bold transition-all shadow-md hover:shadow-lg"
                >
                ...
                <div className="relative group">
                  <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center cursor-help hover:bg-white/30 transition-colors">
